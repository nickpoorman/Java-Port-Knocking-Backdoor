...
public class ContactPanel extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if ("close".equals(e.getActionCommand())) {
			...
			//open a new window for the drop down editor
			//openNewDropDownEditorWindow(statusDropDownID);
			DropDownEditor.showDropDownEditorWindow(statusDropDownID, this.getCompanyID());
		}
	}

	//    public void openNewDropDownEditorWindow(int dropDownID) {
	//        //open the window
	//        DropDownEditor dropDownEditor = new DropDownEditor(dropDownID, this.getCompanyID());
	...
	//        if (cb.equals((JTextField) groupSizeDropDown.getEditor().getEditorComponent())) {
	//            System.out.println("Stage 3");
	//            if (e.getKeyChar() == KeyEvent.VK_DELETE) {
	//                System.out.println("Hit DELETE!");
	//                //popup a window asking if you are sure you want to delete that from the database
	//                int selection = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete that option from the drop down menu?", "Confirm Delete", JOptionPane.YES_NO_OPTION);
	//                if (selection == JOptionPane.YES_OPTION) {
	//                    //remove the current text from the drop down in the database
	//
	//                    // TeaDatabase.deleteDropDownOption(groupSizeDropDownID, cb.getText());
	//                    //then update the component
	//                    //fill the values for the group size drop down
	//                    this.remove(groupSizeDropDown);
	//                    groupSizeDropDownOptions = TeaDatabase.getDropDownOptions(groupSizeDropDownID);
	//                    groupSizeDropDown = new JComboBox(groupSizeDropDownOptions.toArray());
	//                    groupSizeDropDown.getEditor().getEditorComponent().addKeyListener(this);
	//                    groupSizeDropDown.setEditable(true);
	...
	//        companyInfo = TeaDatabase.getCompanyInfo(companyID);
	//        companyInfo.remove(0);
	//        fillValues();
	//    }
	private void save() {
		...
		if (!fields.get(0).getText().equals("")) {
			...
			if (notChanged != false) {
				...
				System.out.println("statusSavedOption: " + statusSavedOption);
				System.out.println("statusDrnSeOption: " + (statusDropDown.getSelectedIndex() + 1));
			...
			if (notChanged) {
				...
				//  statusSavedOption = statusDropDown.getSelectedIndex();
				...
				//        }
				//E         if (this.companyID.equals("-1")) {
				if (this.companyID.isNewCompany()) {
					//insert the new information
					//then pass the list into the insert
					//E               companyID = TeaDatabase.insertCompanyInfo(list);
					...
					//E               TeaDatabase.insertSelectedDropDownsOption(statusDropDown.getSelectedIndex() + 1, statusDropDownID, Integer.parseInt(companyID));
					TeaDatabase.insertSelectedDropDownsOption(statusDropDown.getSelectedIndex() + 1, statusDropDownID,
							companyID.getCompanyID());
					//E                TeaDatabase.insertSelectedDropDownsOption(groupSizeDropDown.getSelectedIndex() + 1, groupSizeDropDownID, Integer.parseInt(companyID));
					...
					System.out.println("updating for company: " + companyID.getCompanyID());
					TeaDatabase.updateForCompanyDetails(this.getCompanyID().getCompanyID(), list);
					openCompaniesTabPane.setTitleAt(openCompaniesTabPane.getSelectedIndex(), fields.get(0).getText());
					//save the status for update
					TeaDatabase.updateSavedDropDowns(statusDropDown.getSelectedIndex() + 1, statusDropDownID, companyID
							.getCompanyID());
				...
				if (!newCompany) {
					//update the contact panel bottom owners
					//save owners
					//            for (int i = 0; i < this.contactPanelBottomOwners.ownersTextFields.size(); i++) {
					//                String ownerName = this.contactPanelBottomOwners.ownersTextFields.get(i).getText();
					//                String percentOwnership = this.contactPanelBottomOwners.percentOwnershipTextFields.get(i).getText();
					//                int ownerID = Integer.parseInt(this.contactPanelBottomOwners.ownerIDs.get(i));
					//
					...
					//save group numbers
					//            for (int i = 0; i < this.contactPanelBottomGroupNumber.groupNumberTextFields.size(); i++) {
					//                String groupNumber = this.contactPanelBottomGroupNumber.groupNumberTextFields.get(i).getText();
					//                int groupNumberID = Integer.parseInt(this.contactPanelBottomGroupNumber.groupNumberIDs.get(i));
					//
	...
	private void createComponents() {
		...
		// groupSizeDropDown.setActionCommand("groupSizeDropDown");
	...
	private void intiLayout() {
		...
		layout.putConstraint(SpringLayout.WEST, groupSizeLabel, 0, SpringLayout.WEST, firstColumnLabels
				.get(firstColumnLabels.size() - 1));
		layout.putConstraint(SpringLayout.NORTH, groupSizeLabel, 30, SpringLayout.SOUTH, firstColumnLabels
				.get(firstColumnLabels.size() - 1));
		...
		//        //set the letf and right bottom pannels
		//        layout.putConstraint(SpringLayout.NORTH, bottomScrollPaneOwners, 15, SpringLayout.SOUTH, groupSizeDropDown);
		//        layout.putConstraint(SpringLayout.NORTH, bottomScrollPaneGroupNumber, 15, SpringLayout.SOUTH, groupSizeDropDown);
		//        layout.putConstraint(SpringLayout.SOUTH, bottomScrollPaneOwners, 0, SpringLayout.SOUTH, this);
		//        layout.putConstraint(SpringLayout.SOUTH, bottomScrollPaneGroupNumber, 0, SpringLayout.SOUTH, this);
		//        //left panel
		//        layout.putConstraint(SpringLayout.WEST, bottomScrollPaneOwners, 0, SpringLayout.WEST, this);
		//        layout.putConstraint(SpringLayout.EAST, bottomScrollPaneOwners, 0, SpringLayout.WEST, bottomScrollPaneGroupNumber);
		//        //right panel
		//        layout.putConstraint(SpringLayout.EAST, bottomScrollPaneGroupNumber, 0, SpringLayout.EAST, this);
		//       // layout.putConstraint(SpringLayout.WEST, bottomScrollPaneGroupNumber, 0, SpringLayout.WEST, bottomScrollPaneOwners);
		...
		//        for (int i = 0; i < labels.size(); i++) {
		//            if (i == 0) {
		//                layout.putConstraint(SpringLayout.WEST, labels.get(i), 20, SpringLayout.WEST, this);
		//                layout.putConstraint(SpringLayout.NORTH, labels.get(i), 20, SpringLayout.NORTH, this);
		//                layout.putConstraint(SpringLayout.WEST, labels.get(i).getLabelFor(), 10, SpringLayout.EAST, labels.get(i));
		//                layout.putConstraint(SpringLayout.SOUTH, labels.get(i).getLabelFor(), 0, SpringLayout.SOUTH, labels.get(i));
		//            } else if (i == 2) {
		//                layout.putConstraint(SpringLayout.WEST, labels.get(i), 0, SpringLayout.WEST, labels.get(i - 1));
		//                layout.putConstraint(SpringLayout.NORTH, labels.get(i), 30, SpringLayout.NORTH, labels.get(i - 1));
		//                layout.putConstraint(SpringLayout.WEST, labels.get(i).getLabelFor(), 10, SpringLayout.EAST, labels.get(i));
		//                layout.putConstraint(SpringLayout.SOUTH, labels.get(i).getLabelFor(), 0, SpringLayout.SOUTH, labels.get(i));
		//            } else if (i == 3) {
		//                layout.putConstraint(SpringLayout.EAST, labels.get(i), 0, SpringLayout.WEST, labels.get(i - 1));
		//                layout.putConstraint(SpringLayout.NORTH, labels.get(i), 0, SpringLayout.NORTH, labels.get(i - 1));
		//                layout.putConstraint(SpringLayout.WEST, labels.get(i).getLabelFor(), 10, SpringLayout.EAST, labels.get(i));
		//                layout.putConstraint(SpringLayout.SOUTH, labels.get(i).getLabelFor(), 0, SpringLayout.SOUTH, labels.get(i));
		//            } else if (i == 4) {
		//                layout.putConstraint(SpringLayout.EAST, labels.get(i), 0, SpringLayout.WEST, labels.get(i - 1));
