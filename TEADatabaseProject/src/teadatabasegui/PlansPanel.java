...
public class PlansPanel extends JPanel implements ActionListener {
    ...
    public boolean checkInput() {
        ...
        if (!verifyDateFormat(submittedDate.getText())) {
            ok = false;
            JOptionPane.showMessageDialog(null, "'Submitted Date' must be in MM/DD/YYYY format");
        }
        ////////////////
        if (!verifyDateFormat(approvalDate.getText())) {
            ok = false;
            JOptionPane.showMessageDialog(null, "'Approval Date' must be in MM/DD/YYYY format");
        }
        ////////////////
        if (!verifyDateFormat(welcomeLetter.getText())) {
            ok = false;
            JOptionPane.showMessageDialog(null, "'Welcome Letter Date' must be in MM/DD/YYYY format");
        }
        ////////////////
        if (!verifyDateFormat(effectiveDate.getText())) {
            ok = false;
            JOptionPane.showMessageDialog(null, "'Effective Date' must be in MM/DD/YYYY format");
        }
        ////////////////
    ...
    public void save() {
        if (!this.companyID.isNewCompany()) {
            ...
            TeaDatabase.updatePlansForCompany(this.companyID.getCompanyID(),
                    union.getText(),
                    RatesPanel.changeDateUserToDatabase(submittedDate.getText()),
                    initials.getText(),
                    RatesPanel.changeDateUserToDatabase(approvalDate.getText()),
                    RatesPanel.changeDateUserToDatabase(welcomeLetter.getText()),
                    RatesPanel.changeDateUserToDatabase(effectiveDate.getText()));

            //save the rating drop down selected option
            TeaDatabase.updateSavedDropDowns(ratingDropDown.getSelectedIndex() + 1, ratingDropDownMenuID, companyID.getCompanyID());

            //save the classes drop down selected option
            TeaDatabase.updateSavedDropDowns(classesDropDown.getSelectedIndex() + 1, classesDropDownMenuID, companyID.getCompanyID());
    ...
    public void fillValues() {
        ...
        //                );
        //make sure the dates are in the user format
        union.setText(data.get(1));
        submittedDate.setText(RatesPanel.changeDateDatabaseToUser(data.get(2)));
        initials.setText(data.get(3));
        approvalDate.setText(RatesPanel.changeDateDatabaseToUser(data.get(4)));
        welcomeLetter.setText(RatesPanel.changeDateDatabaseToUser(data.get(5)));
        effectiveDate.setText(RatesPanel.changeDateDatabaseToUser(data.get(6)));
